...
# File name: daemon.py
# Description: Basic Daemon Module For Python Programs
...
class Daemon(object):
    """
    Usage: - Create your own a subclass Daemon class and override the run() method.
             Run() will be periodically the calling inside the infinite run loop
           - You can receive reload signal from self.isReloadSignal and then you
             have to set back self.isReloadSignal = False
    """
    def __init__(self, stdin='/dev/null', stdout='/dev/null', stderr='/dev/null'):
        self.ver = __version__  # version
        self.pauseRunLoop = 0    # 0 means none pause between the calling of run() method.
        self.restartPause = 1    # 0 means without a pause between stop and start during the restart
        self.waitToHardKill = 3  # wait until kill the process with SIGTERM signal
